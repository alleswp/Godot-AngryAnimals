...
func level_selected(level: int) -> void:
	...
	_attempts = 0
	_cups_hit = 0
	print("level selected:%s level scores:%s" % [
...
func set_target_cups(t: int) -> void:
	_target_cups = t
	print("set target cups:", _target_cups)
	
func attempt_made() -> void:
	_attempts += 1
	SignalManager.on_attempt_made.emit()
	print("on attemps made:%s target cups:%s, cups hit:%s" % [
		_attempts, _target_cups, _cups_hit
	])
...
func on_cup_destroyed() -> void:
	_cups_hit += 1
	print("on cup destroyed:%s target cups:%s, attempts:%s, cups hit:%s" % [
