...
var _last_dragged_amount: float = 0.0
...
func drag_it() -> void:
	...
	_last_dragged_amount = (_last_dragged_position - gmp).length()
	_last_dragged_position = gmp
	
	if _last_dragged_amount > 0 and stretch_sound.playing == false:
		stretch_sound.play()
